...
pub async fn clear(
    ctx: Context<'_>,
    #[description = "How many msg to clear"]
    #[min = 1]
    count: Option<u8>,
) -> Result<(), Error> {
    ...
    if count > 100 {
        ...
        msg.append(
            c.messages(
                ...
                GetMessages::new().limit(count).before(msg[0].id),
            )
            .await?
            .as_mut(),
        );
...
fn create_channel_builder<'a>(
    ...
    let mut cc = poise::serenity_prelude::CreateChannel::new(name)
    ...
    match channel_id.to_channel(&serenity_context).await? {
        Channel::Guild(guild_channel) => {
            let guild = guild_channel
                .guild_id
                .to_partial_guild(&serenity_context)
                .await?;
            let name = guild_channel.name.clone();

            // 傳入字串切片，避免引用本地建立的 String
            let cc = create_channel_builder(&guild_channel, name, why.map(|s| s.as_str()));
            ...
            new.send_message(
                &*serenity_context,
                poise::serenity_prelude::CreateMessage::default().embed(
                    poise::serenity_prelude::CreateEmbed::new()
                        .author(author.into())
                        .title("nuke channel")
                        .description(r)
